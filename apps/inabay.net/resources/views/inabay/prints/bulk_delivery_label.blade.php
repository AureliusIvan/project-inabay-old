...
<div class="wrap">
    <div class="top">
        @if($sales->delivery_logo)
            <div>
                <div>
                    <div style="position: absolute; top:0; left:0;">
                        <img height="25" style="max-width: 100px;" src="{{ $sales->delivery_logo }}" />
                    </div>
                    <div class="title" style="margin-top: 5px; margin-left: 115px; {{$sales->delivery_barcode ? "":"padding-bottom:20px;"}}">
                        Tiket: {{$current_date}} {{$sales->trx_no}}
                        <hr />
                    </div>
                </div>
                @if($sales->delivery_barcode)
                    <div style="clear:both; text-align: center; margin-top: 5px; margin-bottom: 8px;">
                        <img src="{{$sales->delivery_barcode}}" alt="barcode" />
                        <div style="margin-top:10px;"><b>{{$sales->booking_code}}</b></div>
                    </div>
                @endif
            </div>
        @else
            <div class="title" style="text-align:center;">
                ...
                @if($sales->delivery_barcode)
                    <div style="padding-top: 10px;">
                        <img style="margin:0; padding:0;" src="{{$sales->delivery_barcode}}" alt="barcode" />
    ...
    <div class="group" style="clear:both;{{$sales->delivery_logo ? "margin-top: -8px":""}}">
        <div>Kepada:</div>
    ...
    <div class="group font-kecil">
        Isi Barang:
        @foreach($sales->sales_detail as $sales_detail)
